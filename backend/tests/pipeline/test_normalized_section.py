...
class TestFlatNotesParser:
    ...
    def test_flat_effective_date_of_amendment_note_is_statutory(self) -> None:
        """'Effective Date of <year> Amendment' notes are classified as STATUTORY.

        Issue #686: the overbroad _EFFECTIVE_DATE_PATTERN (added in #652) was
        classifying ALL "Effective Date of ..." headers as EDITORIAL, but only
        "Effective Date of Repeal" (which is in EDITORIAL_HEADERS) should be
        EDITORIAL.  Year-specific variants like "Effective Date Of 1986 Amendment"
        appear under "Statutory Notes and Related Subsidiaries" in OLRC and must
        therefore be STATUTORY.
        """
        from pipeline.olrc.normalized_section import SectionNotes, _parse_flat_notes
    ...
    def test_flat_effective_date_of_1990_amendment_is_statutory(self) -> None:
        """'Effective Date of 1990 Amendment' header is classified as STATUTORY.

        Issue #686: confirms the fix for the overbroad _EFFECTIVE_DATE_PATTERN
        applies to year-specific effective-date-of-amendment variants.
        """
        from pipeline.olrc.normalized_section import SectionNotes, _parse_flat_notes
    ...
    def test_flat_effective_date_of_amendment_is_statutory_not_editorial_issue_686(
        self,
    ) -> None:
        """'Effective Date of 1976 Amendment' in flat notes is STATUTORY, not EDITORIAL.

        Regression test for issue #686: discovered against 13 U.S.C. § 241.  The
        overbroad _EFFECTIVE_DATE_PATTERN (introduced to fix #652) classified ALL
        "Effective Date of ..." flat-notes headers as EDITORIAL.  Only the
        explicitly-listed "Effective Date of Repeal" entry in EDITORIAL_HEADERS
        should be EDITORIAL; all other effective-date-of-amendment notes appear
        under "Statutory Notes and Related Subsidiaries" in OLRC and must be
        STATUTORY.  Also verifies that "Effective Date of Repeal" is still
        EDITORIAL (regression guard).
        """
        from pipeline.olrc.normalized_section import (
            NoteCategory,
            SectionNotes,
        ...
        raw_notes = (
            "[NH]Historical and Revision Notes[/NH] "
            "Based on title 13, U.S.C., 1952 ed., § 241 (Mar. 6, 1902, ch. 139, § 2). "
            "[NH]Amendments[/NH] "
            "1976—Pub. L. 94–521, § 14, Oct. 17, 1976, 90 Stat. 2467, added subsec. (b). "
            "[NH]Effective Date of 1976 Amendment[/NH] "
            "Amendment by Pub. L. 94–521 effective Oct. 1, 1976, see section 17 of Pub. L. 94–521. "
        )
        notes = SectionNotes()
        _parse_flat_notes(raw_notes, notes)

        category_by_header = {n.header: n.category for n in notes.notes}

        # Core fix: "Effective Date of 1976 Amendment" must be STATUTORY
        assert "Effective Date of 1976 Amendment" in category_by_header, (
            "Expected note 'Effective Date of 1976 Amendment' to be parsed"
        )
        assert (
            category_by_header["Effective Date of 1976 Amendment"]
            == NoteCategory.STATUTORY
        ), (
            f"Expected STATUTORY, got {category_by_header['Effective Date of 1976 Amendment']!r}"
        )

        # Historical note must still be HISTORICAL
        assert (
            category_by_header.get("Historical and Revision Notes")
            == NoteCategory.HISTORICAL
        ), "Expected 'Historical and Revision Notes' to be HISTORICAL"

        # Amendments must still be EDITORIAL
        assert category_by_header.get("Amendments") == NoteCategory.EDITORIAL, (
            "Expected 'Amendments' to be EDITORIAL"
        )

    def test_flat_effective_date_of_repeal_is_still_editorial_issue_686(self) -> None:
        """'Effective Date of Repeal' remains EDITORIAL after removing _EFFECTIVE_DATE_PATTERN.

        Regression guard for issue #686: EDITORIAL_HEADERS explicitly lists
        "Effective Date of Repeal", so removing the broad _EFFECTIVE_DATE_PATTERN
        must not change its classification.
        """
        from pipeline.olrc.normalized_section import (
            NoteCategory,
            SectionNotes,
        ...
        raw_notes = (
            "[NH]Effective Date of Repeal[/NH] "
            "Pub. L. 107–273, div. C, title III, § 13207, Nov. 2, 2002, 116 Stat. 1905, "
            "provided that the repeal shall take effect upon the effective date of such rule."
        )
        notes = SectionNotes()
        _parse_flat_notes(raw_notes, notes)

        assert len(notes.notes) == 1
        assert notes.notes[0].header == "Effective Date of Repeal"
        assert notes.notes[0].category == NoteCategory.EDITORIAL, (
            f"Expected EDITORIAL, got {notes.notes[0].category!r}"
        )
...
class TestTitle17Section106Notes:
    ...
    def test_effective_date_of_amendment_notes_are_statutory(self) -> None:
        """'Effective Date of <year> Amendment' notes are classified as STATUTORY.

        Issue #686: the overbroad _EFFECTIVE_DATE_PATTERN (added in #652) classified
        ALL "Effective Date of ..." headers as EDITORIAL.  These notes appear under
        "Statutory Notes and Related Subsidiaries" in OLRC (topic="effectiveDateOfAmendment"
        in USLM XML), so they must be STATUTORY.  Only "Effective Date of Repeal"
        (explicitly listed in EDITORIAL_HEADERS) remains EDITORIAL.
        """
        notes = self._parse()
...
class TestNoteTopicAmendmentParsing:
    ...
    def test_multiple_flat_notes_mixed_categories_issue_222(self) -> None:
        ...
        assert (
            len(editorial) == 2
        )  # References in Text + Amendments (issue #686: Effective Date of Amendment is now STATUTORY)
        assert (
            len(statutory) == 2
        )  # Effective Date of 1996 Amendment + No Requirement of Reimbursement
