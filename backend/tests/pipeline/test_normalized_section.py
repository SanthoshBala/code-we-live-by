...
class TestCitationParsing:
    ...
    def test_parsed_citation_public_law_id_property(self) -> None:
        """Test the public_law_id property format."""
        citation = ParsedCitationData(congress=118, law_number=60)
        assert citation.public_law_id == "PL 118-60"

    def test_parsed_citation_data_fields(self) -> None:
        """Test ParsedCitationData fields are accessible."""
    ...
    def test_citations_returned_in_order(self) -> None:
        """Test that citations are returned in chronological order (list index = order)."""
        text = """( Pub. L. 94–553, title I, § 106, Oct. 19, 1976, 90 Stat. 2546;
        Pub. L. 101–650, title VII, § 703, Dec. 1, 1990, 104 Stat. 5133;
        Pub. L. 106–44, § 2, Aug. 5, 1999, 113 Stat. 221 )"""
        ...
        # First citation (index 0) is the original/creating law
        assert citations[0].congress == 94
        assert citations[0].law_number == 553

        # Subsequent citations are amendments in order
        assert citations[1].congress == 101
        assert citations[1].law_number == 650
    ...
    def test_parsed_citation_sort_key(self) -> None:
        """Test that sort_key provides chronological ordering."""
        # Create citations out of order
        c1 = ParsedCitationData(congress=106, law_number=44)
        c2 = ParsedCitationData(congress=94, law_number=553)
        c3 = ParsedCitationData(congress=101, law_number=650)

        # Sort by (congress, law_number) should give chronological order
        citations = sorted([c1, c2, c3], key=lambda c: (c.congress, c.law_number))
