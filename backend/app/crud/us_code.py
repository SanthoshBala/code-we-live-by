...
async def get_section(
    ...
    # Inherit is_positive_law from the title-level SectionGroup row
    title_stmt = select(SectionGroup).where(
        SectionGroup.group_type == "title",
        SectionGroup.number == str(title_number),
    )
    title_result = await session.execute(title_stmt)
    title_group = title_result.scalar_one_or_none()
    is_positive_law = title_group.is_positive_law if title_group is not None else False

    # Derive dates from notes when available
    enacted_date = None
