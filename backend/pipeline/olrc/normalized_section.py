...
@dataclass
class ParsedCitationData:
    """Raw citation data parsed from legal text.

    This class holds the raw parsed data from citation text before
    linking to a PublicLaw database record. Use this during text parsing,
    then convert to Citation once the PublicLaw is resolved.
    ...
    def to_citation(self, public_law: PublicLaw, order: int = 0) -> "Citation":
        """Convert to a Citation with a resolved PublicLaw reference.

        Args:
            public_law: The resolved PublicLaw database record.
            order: Position in citation list (0 = original/creating law).

        Returns:
            A Citation instance with the PublicLaw reference.
        """
        return Citation(
            ...
            stat_page=self.stat_page,
            raw_text=self.raw_text,
            order=order,
        )

    def __repr__(self) -> str:
        return f"<ParsedCitationData({self.public_law_id})>"
...
class Citation:
    ...
    This class references the PublicLaw model rather than embedding
    congress/law_number fields directly. The public_law field provides
    access to the full law record including enacted_date, popular_name, etc.

    Example: "Pub. L. 94–553, title I, § 101, Oct. 19, 1976, 90 Stat. 2546"
    """

    public_law: PublicLaw  # Reference to the PublicLaw model
    title: str | None = None  # Title within the law, e.g., "I"
    section: str | None = None  # Section within the law, e.g., "101"
    date: str | None = None  # Enactment date, e.g., "Oct. 19, 1976"
    stat_volume: int | None = None  # Statutes at Large volume, e.g., 90
    ...
    def congress(self) -> int:
        """Return the congress number from the referenced PublicLaw."""
        return self.public_law.congress

    @property
    def law_number(self) -> str:
        """Return the law number from the referenced PublicLaw."""
        return self.public_law.law_number
...
def parse_citation(text: str) -> ParsedCitationData | None:
    """Parse a citation string into a ParsedCitationData object.

    Args:
        text: Raw citation text like "Pub. L. 94–553, title I, § 101, Oct. 19, 1976, 90 Stat. 2546"

    Returns:
        ParsedCitationData object or None if parsing fails.
        Use ParsedCitationData.to_citation() to convert to a Citation
        once the PublicLaw is resolved from the database.
    """
...
def parse_citations(text: str) -> list[ParsedCitationData]:
    ...
    The list index corresponds to the order (0 = original/creating law).

    Args:
        text: Text containing one or more citations.

    Returns:
        List of ParsedCitationData objects in chronological order.
        Use ParsedCitationData.to_citation() to convert each to a Citation
        with the resolved PublicLaw, passing the list index as the order.
    """
    citations: list[ParsedCitationData] = []
...
) -> list[ParsedCitationData]:
    """Convert structured SourceCreditRef objects to ParsedCitationData objects.
...
class SectionNotes:
    ...
    # Uses ParsedCitationData since PublicLaw records are resolved during ingestion.
    citations: list[ParsedCitationData] = field(default_factory=list)
...
def _parse_notes_structure(
    raw_notes: str,
    notes: SectionNotes,
    citations: list[ParsedCitationData] | None = None,
) -> None:
