...
- **PR screenshots**: After creating a PR that includes frontend/UI changes, post screenshots of the affected pages. Both servers must be running first: backend on port 8000 (`cd backend && uv run uvicorn app.main:app --port 8000`) and frontend on port 3000 (`cd frontend && npm run dev`). If port 3000 is already occupied by a stale process, kill it: `lsof -ti :3000 | xargs kill -9`. Then run `bash scripts/post_pr_screenshots.sh <PR_NUMBER> /path1 /path2 ...`, passing every page path that shows the new or changed UI. The script uploads screenshots to the PR branch, posts a comment with commit-SHA-pinned image URLs, then deletes the uploaded files—no external image hosting needed.
